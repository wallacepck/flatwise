...
class InsightGenerator:
    ...
    def query_top_k_var(self, evidence: dict, top_k=3, variable='resale_price') -> list[tuple[float, Any]]:
        ...
        # Handle top_k=0 to return all values
        if top_k == 0:
            topk_indexes = query.values.argsort()[::-1] # Get all indexes, sorted descending
        else:
            topk_indexes = query.values.argsort()[-top_k:]
    ...
    def get_insights_on_row(self, row: pd.Series) -> Dict[str, Any]:
        """
        NEW: Returns a dictionary with all 3 tiers AND one featured text insight.
        """
        row = convert_numeric_to_interval(row, self.categories)
        ...
        # Run all three insight functions
        all_insights = []
        
        try:
            all_insights.append(self.insight_over_gte_lease(evidence))
        except InsufficentDataError:
            all_insights.append(("Average", "Competitive value for its lease.")) # Default
        except Exception:
             all_insights.append(("Average", "Competitive value for its lease."))

        try:
            all_insights.append(self.insight_price_due_lease_depreciation(evidence))
        except InsufficentDataError:
            all_insights.append(("Average", "Standard resale risk.")) # Default
        except Exception:
            all_insights.append(("Average", "Standard resale risk."))

        try:
            all_insights.append(self.insight_floor_area(evidence))
        except InsufficentDataError:
            all_insights.append(("Average", "Average floor area for its price.")) # Default
        except Exception:
            all_insights.append(("Average", "Average floor area for its price."))

        # Build the structured dictionary
        tiers = {
            "lease_value": all_insights[0][0],
            "resale_risk": all_insights[1][0],
            "size_value": all_insights[2][0]
        }
        
        # Get all the long-form text
        text_insights = [insight[1] for insight in all_insights]
        
        # Return the tiers AND one randomly selected long-form text
        return {
            "tiers": tiers,
            "text": random.choice(text_insights)
        }
        
    def insight_over_gte_lease(self, evidence: dict) -> Tuple[str, str]:
        baseline_lease = evidence['remaining_lease_years']
        baseline_price = evidence['resale_price'].mid
        ...
        max_confidence = 0
        for lease in get_lease_cats(self.categories['remaining_lease_years'], baseline_lease, 'gte'):
            sample_evidence['remaining_lease_years'] = lease
            sample_flat = self.query_top_k_var(sample_evidence, top_k=1)
            if sample_flat[0][0] < STATISTICAL_CUTOFF:
                continue
            if sample_flat[0][1].mid < baseline_price and sample_flat[0][0] > max_confidence:
        ...
        given_data = f" among {evidence['flat_type']} flats in {evidence['town']} with >{int(evidence['remaining_lease_years'].left)} years lease"
        if max_confidence > 0.3:
            return ("Bad", "Not competitive value" + given_data)
        elif max_confidence >= STATISTICAL_CUTOFF:
            return ("Average", "Competitive value" + given_data)
        else:
            return ("Good", "Best value" + given_data)
            
    def insight_price_due_lease_depreciation(self, evidence: dict) -> Tuple[str, str]:
        sample_evidence = {
            'town': evidence['town'],
            'flat_model': evidence['flat_model'],
            'flat_type': evidence['flat_type'],
        ...
        if not topk_prob_prices or sum(p for p, v in topk_prob_prices) < STATISTICAL_CUTOFF:
            raise InsufficentDataError("Not enough data for volatility calculation")
        ...
        for year in depreciated_years:
            ...
            sample_evidence['remaining_lease_years'] = year
            topk_prob_prices = self.query_top_k_var(sample_evidence, top_k=0)

            if not topk_prob_prices or topk_prob_prices[0][0] < STATISTICAL_CUTOFF:
                continue
        ...
        if max_volatility > base_volatility:
            delta = max_volatility - base_volatility
            if delta > 50_000:
                return ("Bad", f"Large impact on resale potential due to lease decay - High change in volatility (+{int(delta)}) in ~{int(years_to_dep)} years")
            else:
                return ("Average", "Minimal impact on resale potential due to lease decay for next 20 years")
        else:
            return ("Good", "No impact on resale potential due to lease decay for next 20 years")


    def insight_floor_area(self, evidence: dict) -> Tuple[str, str]:
        sample_evidence = {
            'town': evidence['town'],
            'flat_model': evidence['flat_model'],
            'flat_type': evidence['flat_type'],
        ...
        if not topk_prob_sqm or topk_prob_sqm[0][0] < STATISTICAL_CUTOFF:
            raise InsufficentDataError()
        ...
        for prob, sqm in topk_prob_sqm:
            ...
            if evidence['floor_area_sqm'].mid >= sqm.left and evidence['floor_area_sqm'].mid < sqm.right:
                return ("Average", f"Average floor area (~{int(avg_sqm.mid)} sqm) in this price range")
        
        if avg_sqm.right > evidence['floor_area_sqm'].mid:
            return ("Bad", f"Lower than average floor area (~{int(avg_sqm.mid)} sqm) in this price range")
        else:
            return ("Good", f"Higher than average floor area (~{int(avg_sqm.mid)} sqm) in this price range")